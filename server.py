...
async def push_notify_credit(
    ...
    # â see _sl_try_auto_enter below.
    if (
...
async def sl_p2p_pool_confirm(
    ...
    Speaking Lab treasury as a pool entry.

    Phase 1: ADMIN-ONLY. Not called by the PWA yet — the existing
    /api/push/notify-credit hook continues to drive auto-entry from
    My Portal P2P. Mounted here so a future Phase can wire the PWA to
    call this explicit endpoint without further backend work.

    The recipient must be the configured treasury (SL_TREASURY_ID); any
    other recipient is rejected with status="rejected_wrong_recipient"
    and NO pool entry is created (P2P transfer itself is unaffected —
    it already happened in GAS).
    """
...
async def sl_pool_diagnostics(
    ...
    """Return accepted / repaired / rejected / warned / duplicate events
    for this session so the operator can answer "why didn't stu004 enter
    the pool?" or "why is the lucky code missing?".

    Read-only; safe to poll. ADMIN-ONLY.
    """
    sess = await SL_SESSIONS.find_one({"session_id": session_id})
    if not sess:
        raise HTTPException(status_code=404, detail="Session not found")
    ...
    # Also surface entries that exist on the roster but have no lucky
    # code yet — these are repairable with a re-trigger of
    # generate_and_publish_lucky_code() and should rarely be > 0 once
    # this patch is live.
    ...
    # Bucket events for the operator UI.
